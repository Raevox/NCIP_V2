...
.partner-table tbody tr { transition: background 0.2s ease; }
.partner-table tbody tr:hover { background: #f8fdf5; }
...
.sector-government { background: #eff6ff; color: #1d4ed8; }
.sector-private    { background: #faf5ff; color: #7c3aed; }
...
.partner-name-text { font-weight: 600; color: var(--text-dark); }
...
<div class="partners-content">
    ...
    <div class="page-header">
        <h2>
            <span class="header-icon"><i class="fas fa-handshake"></i></span>
            Partner Management
        </h2>
        <a href="{{ route('admin.partners.create') }}" class="btn-add-partner" id="addPartnerBtn">
    ...
    <div class="controls-bar">
        <div class="controls-left">
            ...
            <div class="sector-filter">
                <i class="fas fa-filter" style="color: var(--text-soft); font-size: 13px;"></i>
                <select id="sectorFilter">
                    <option value="">All Sectors</option>
                    <option value="government" {{ $sector === 'government' ? 'selected' : '' }}>Government</option>
                    <option value="private"    {{ $sector === 'private'    ? 'selected' : '' }}>Private / CSO</option>
                </select>
            </div>
        </div>
        ...
        <div class="stats-pill">
            ...
            <strong id="partnerCount">{{ $partners->total() }}</strong>
            <span>Partner(s) found</span>
        </div>
    </div>

    {{-- Dynamic Table Container --}}
    <div id="tableContainer">
        @include('admin.partners.partials.table')
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold" style="font-family:'Poppins',sans-serif;">Delete Partner</h5>
            ...
            <div class="modal-body text-center">
                ...
                <p style="font-size: 13.5px; color: var(--text-mid);">
                    You are about to permanently delete partner <strong id="deletePartnerName"></strong>.
...
// ── Live AJAX Search + Filter + Pagination ──────────────────
let searchTimer;
const searchInput   = document.getElementById('searchInput');
const sectorFilter  = document.getElementById('sectorFilter');
const searchSpinner = document.getElementById('searchSpinner');

function fetchPartnersData(url = '{{ route("admin.partners.index") }}') {
    searchSpinner.style.display = 'block';
    const queryUrl = new URL(url, window.location.origin);
    queryUrl.searchParams.set('search', searchInput.value.trim());
    queryUrl.searchParams.set('sector', sectorFilter.value);
    ...
    .then(data => {
        ...
        document.getElementById('partnerCount').textContent = data.total;
    })
    .catch(() => {
        searchSpinner.style.display = 'none';
    });
...
searchInput.addEventListener('input', function () {
    ...
    searchTimer = setTimeout(() => fetchPartnersData(), 350);
});

sectorFilter.addEventListener('change', () => fetchPartnersData());
